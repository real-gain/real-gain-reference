...
let url = 'http://localhost:8080/tri-weather';
let serverUrl = `${url}/mcp`;
...
function printHelp(): void {
  console.log('\nAvailable commands:');
  console.log('  connect [url]              - Connect to MCP server (default: http://localhost:8080/tri-weather/mcp)');
  console.log('  disconnect                 - Disconnect from server');
  console.log('  terminate-session          - Terminate the current session');
  console.log('  reconnect                  - Reconnect to the server');
  console.log('  list-plans                 - List available plans');
...
const listPlans = async (): Promise<void> => {
  const response = await fetch(`${url}/real-gain/plans`);
  ...
  try {
    ...
    result.content.forEach(item => {
      if (item.type === 'text') {
        ...
        console.log(`  ${item.name}:`, item.resource);
      } else {
